...
% 要了解详细信息，请参阅 从深度网络设计器生成 MATLAB 代码。
% 由 MATLAB 于 2023-10-19 11:00:08 自动生成
% 创建层次图
% 创建层次图变量以包含网络层。
function lgraph = GoogleNet()
lgraph = layerGraph();
% 
% 添加层分支
% 将网络分支添加到层次图中。每个分支均为一个线性层组。
tempLayers = [
    imageInputLayer([32 32 3],"Name","data")
...
lgraph = addLayers(lgraph,tempLayers); % 将这一行代码将定义的这些层添加到一个层图中。lgraph是一个layerGraph对象，它表示整个神经网络的层结构。
...
tempLayers = depthConcatenationLayer(4,"Name","inception_3a-output");%%深度方向串联
lgraph = addLayers(lgraph,tempLayers);
...
tempLayers = [
    ...
    fullyConnectedLayer(10,"Name","loss3-classifier","BiasLearnRateFactor",2)
